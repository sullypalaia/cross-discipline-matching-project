...
export default async function DashboardPage() {
  ...
  const ownerIds = Array.from(new Set(projects.map((project) => project.owner).filter(Boolean)));
  ...
  const ownerNameById = new Map(ownerNames.map((owner) => [owner.id, owner.username]));
      ...
      <section className="mx-auto max-w-7xl px-5 py-10 sm:px-8 sm:py-14">
        <div className="max-w-2xl">
          ...
          <p className="mt-3 text-slate-600">
            Keep track of your active collaborations and the join requests waiting
            for a decision.
          </p>
        </div>
        ...
        <div className="mt-10 space-y-10">
              ...
              return (
                <article key={project.proj_id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="flex items-start justify-between gap-3">
                    <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${isOwner ? "bg-indigo-50 text-indigo-700" : "bg-emerald-50 text-emerald-700"}`}>
                      {isOwner ? "Your project" : "Joined project"}
                    </span>
                    <span className="text-xs text-slate-500">{project.num_members ?? 1} members</span>
                  </div>
                  <h3 className="mt-4 text-xl font-bold text-slate-950">{projectName(project)}</h3>
                  <p className="mt-2 text-sm text-slate-500">
                    Owner: <span className="font-semibold text-slate-700">{isOwner ? "You" : project.owner_name || "Project owner"}</span>
                  </p>
          ...
          <DashboardSection
            eyebrow="Your outgoing requests"
            ...
            {pendingRequests.map((request) => {
              ...
              return (
                <article key={request.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="flex items-start justify-between gap-3">
                    <h3 className="font-bold text-slate-950">{project ? projectName(project) : "Project unavailable"}</h3>
                    <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${statusStyles(request.status)}`}>Pending</span>
                  </div>
                  <p className="mt-2 text-sm leading-6 text-slate-600">Your request is with the project owner for review.</p>
                  <p className="mt-3 text-sm text-slate-600">
                    Owner: <span className="font-semibold text-slate-700">{project?.owner === user.id ? "You" : project?.owner_name || "Project owner"}</span>
                  </p>
            ...
            {pendingIncomingRequests.map((request) => {
              ...
              return (
                <article key={request.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <h3 className="font-bold text-slate-950">{request.applicant_name?.trim() || "A collaborator"}</h3>
                      <p className="mt-1 text-xs text-slate-500">Wants to join {project ? projectName(project) : "your project"}</p>
                    </div>
                    <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${statusStyles(request.status)}`}>Pending</span>
                  </div>
                  {request.motivation && <p className="mt-4 text-sm leading-6 text-slate-600">{request.motivation}</p>}
                  {request.contribution && <p className="mt-3 text-sm text-slate-700"><span className="font-semibold">Can help with:</span> {request.contribution}</p>}
                  <p className="mt-4 text-xs text-slate-500">
                    {request.hours_per_week ?? "—"} hrs/week · {request.meeting_modality ?? "Modality not specified"}
                  </p>
