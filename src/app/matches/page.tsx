...
export default async function MatchesPage() {
  ...
  const [{ data: profile }, { data: projects, error: projectsError }] =
    await Promise.all([
      supabase
        .from("profiles")
        .select("display_name, skills, interests, hours_per_week")
        .eq("id", user.id)
        .maybeSingle(),
  ...
  const initialProfile: Profile | null =
    (
    ...
    ) ?
      {
        ...
        hoursPerWeek: Number(profile.hours_per_week),
      }
    : null;
  ...
  return (
    ...
    >
      ...
      <div className="relative min-h-64 overflow-hidden bg-slate-950 text-white sm:min-h-80">
        <img
          src="https://www.sanluisobispo.com/public/latest-news/ka5ls9/picture316451279/alternates/LANDSCAPE_1200/SLO_Cal%20Poly%20Synthetic%20Cells.JPG"
          alt="Cal Poly students collaborating outdoors"
        ...
        <div className="relative z-10 mx-auto flex min-h-64 max-w-7xl flex-col justify-center px-5 py-8 sm:min-h-80 sm:px-8">
          ...
          <p className="mt-4 max-w-2xl text-sm leading-6 text-slate-100 sm:text-base">
            Set the skills, interests, and time you can offer. We’ll compare
            them with active projects and suggest strong fits.
          </p>
        </div>
      </div>
